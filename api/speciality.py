...
speciality_bp = Blueprint("speciality", __name__)

@speciality_bp.route("/specialities", methods=["GET"])
def get_all_specialities():
    specialities = Speciality.query.all()
    result = [{"id": s.id, "name": s.name} for s in specialities]
    return jsonify(result), 200

@speciality_bp.route("/procedures", methods=["GET"])
def get_procedures_by_speciality():
