...
test_data = pd.read_csv('data/test/test_data.csv', low_memory=False)
...
for proc_id in test_data['procedure_id'].unique():
    ...
    proc_encoded = test_encoded[test_data['procedure_id'] == proc_id]
    
    # Get default and surgeon-specific materials
    default_data = proc_data[proc_data['is_default'] == 1]
    default_materials = set(default_data['material_name'])
    surgeon_data = proc_data[proc_data['is_default'] == 0]
    surgeon_added = set(surgeon_data[surgeon_data['surgeon_specific_action'] != 'default']['material_name']) - default_materials
    ...
    # Optimize materials based on predicted prices
    optimized_materials = {}
    for mat in all_materials:
        mat_rows = proc_data[proc_data['material_name'] == mat]
        if not mat_rows.empty:
            mat_indices = proc_data.index[proc_data['material_name'] == mat].tolist()
            pred_prices = [proc_encoded['predicted_price'].iloc[test_data.index.get_loc(idx)] for idx in mat_indices]
            optimized_materials[mat] = min(pred_prices) if pred_prices else float('inf')
    
    optimized_cost = sum(price for price in optimized_materials.values() if price != float('inf'))
    results[proc_id] = {
        'optimized_materials': optimized_materials,
        'optimized_cost': optimized_cost
    }
...
results_df.to_csv("data/test_optimization_results.csv", index=False)