...
#     # print(f"Procedure ID {proc_id} - Columns in proc_encoded:", proc_encoded.columns.tolist())
...
print("Test MSE:", mean_squared_error(y_test, y_pred_test_sel))
print("Test R² Score:", r2_score(y_test, y_pred_test_sel))
...
for proc_id in data['procedure_id'].unique():
    ...
    proc_encoded = pd.get_dummies(proc_data, columns=['material_name', 'surgeon_specific_action'], prefix=['mat', 'action'])
    proc_X = proc_encoded.reindex(columns=feature_cols, fill_value=0)
    proc_X_scaled = scaler.transform(proc_X)
...
specific_proc_id = 1901  # Hardcoded for now
...
# # Save results to current directory to avoid permission issues
# pd.DataFrame(results).T.to_csv("data/results/ridge_optimization_results_1.csv")
